...
async def test_user_preferences(
    ...
    async with authenticated_notion_api_server:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            preferences = await client.user.async_preferences()
            assert preferences.user_id == 12345
            assert preferences.military_time_enabled is False
            assert preferences.celsius_enabled is False
            assert preferences.disconnect_alerts_enabled is True
            assert preferences.home_away_alerts_enabled is False
            assert preferences.battery_alerts_enabled is True
